...
#define NUMCONTEXTS 5              /* how many contexts to make */
...
*/
void
mkcontext(ucontext_t *uc,  int n, int x)
{   
    ...
    if(n==0){
        makecontext(uc, thread2, 1);
    }
    else{
        /* setup the function we're going to, and n-1 arguments. */
        makecontext(uc, function1, 3 , n, x);
...
int main(int argc, char *argv[]){
    ...
    mkcontext(&contexts[0], 0, 0);
    for(i=1; i < NUMCONTEXTS; i++)
        mkcontext(&contexts[i], i, input[i-1]);
